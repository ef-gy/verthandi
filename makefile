...
PCCFLAGS:=$(shell $(PKGCONFIG) --cflags $(LIBRARIES) 2>/dev/null) -stdlib=libc++
PCLDFLAGS:=$(shell $(PKGCONFIG) --libs $(LIBRARIES) 2>/dev/null) -lboost_system -lsqlite3
...
DATAFILES:=include/verthandi/data-sqlite-verthandi.h
...
clean:
	rm -f $(DATABASES) $(BINARIES) $(DATAFILES); true
scrub: clean
...
include/verthandi/data-sqlite-%.h: src/sqlite3/%.sql
	echo '/* autogenerated data file, based on $< */' > $@
	echo "namespace verthandi { namespace data { namespace sqlite { static const char $*[] = \"$$(sed -e ':a;N;$$!ba;s/\n/\\\\n/g' -e 's/"/\\"/g' $<)\"; } } }" >> $@
